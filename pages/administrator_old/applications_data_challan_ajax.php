...
?>
<table class="table table-hover datatable bg-white table-responsive"  style="font-size: 11px; max-width: fit-content;" data-page-length="100">
...
<table  class="table table-hover datatable bg-white table-responsive w-100"  style="font-size: 11px; max-width: fit-content;" data-page-length="100" >
  <thead class="bg-dark printColor">
    <tr>
      <th width="6%">S.No</th>
      <th>Name</th>
  ...
  <tbody>
    <?php
      $fetchData = "SELECT cp.id AS apply_id, c.id AS cand_id, c.name, c.cnic, c.phone, c.f_name, c.gender, c.image, ct.c_name, cp.apply_date, cp.status,
      cp.status_details, cp.challan_file, cp.challan_upload_date FROM candidate_applied_post AS cp INNER JOIN candidates AS c ON c.id = cp.candidate_id INNER JOIN projects_posts AS pp ON pp.id = cp.post_id LEFT JOIN city AS ct ON ct.id = cp.city_id WHERE cp.post_id = '$postId' AND cp.status = '$candStatus' AND ct.id = '$city_id' ORDER BY ct.c_name, cp.id ASC";
      $runQ = mysqli_query($connection,$fetchData);
      $count = 0;
      while ($rowQ = mysqli_fetch_array($runQ)) {
        $count++;
        ...
        $challan_upload_date = date("d-m-Y",strtotime($rowQ['challan_upload_date']));
    ?>
    <tr style="align-content:center; vertical-align:middle; text-align:center; align-items:center ;">
      <td>
        <?php echo $count ?>
        <input type="hidden" id="autoInc<?php echo $count ?>" value="<?php echo $count ?>">
        <input type="hidden" id="applyId<?php echo $count ?>" value="<?php echo $applyId ?>">
...
<!-- export to csv -->
<table class=" w-100 " style="font-size: 11px; display: none" id="export_table">
  <thead class="bg-dark printColor">
    <tr>
      <th width="6%">S.No</th>
      <th>Name</th>
...
?>
<table  class="table table-hover datatable bg-white table-responsive w-100"  
style="font-size: 12px; max-width: fit-content;" data-page-length="100" >
  <thead class="bg-dark printColor">
    <tr>
      <th width="6%">S.No</th>
      <th>Name</th>
...
<!-- export to csv -->
<table style="font-size: 11px; display: none; max-width: fit-content;" id="export_table">
  <thead class="bg-dark printColor">
    <tr>
      <th>S.No</th>
      <th>Name</th>
