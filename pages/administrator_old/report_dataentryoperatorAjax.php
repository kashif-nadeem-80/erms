...
<table class="table table-hover table-bordered datatable bg-white" data-page-length="100">
  ...
  <tbody>
    <?php
      ...
      $fetchData = "SELECT COUNT(c.id) AS total, m.name, m.contact FROM candidate_applied_post AS c INNER JOIN projects_posts AS pp ON pp.id = c.post_id INNER JOIN projects AS p ON p.id = pp.project_id INNER JOIN management_users AS m ON m.id = c.operator_id WHERE p.id = '$proj_id' GROUP BY c.operator_id ORDER BY m.name ASC";
