  ...
  <tbody>
    <?php
      ...
      else
      {
        if($gender == 'Male')
        {
          $fetchData = "SELECT pw.`written_marks`, pw.`written_weightage`, pw.`typing_marks`, pw.`typing_weightage`, pw.`shorthand_marks`, pw.`shorthand_weightage`, c.`name`, c.`cnic`, c.`email`, c.`phone`, c.`password`, c.`image`, c.`status`, c.`signUpDate`, c.`f_name`, c.`gender`, c.`disability`, c.`disable_file`, c.`dob`, c.`city`, c.`postal_address`, c.`telephone`, c.`religion`, c.`gov_employee`, c.`simple_exper`, c.`retired_pak`, c.`army_exper`, c.`widow_gov_emp`, c.`widow_file`, c.`created_by`, c.`marital_status`, c.`father_occup`, c.`f_contact`, d.dis_name, r.roll_no, r.written_result, r.typing_result, r.shorthand_result, r.physical_result, pp.post_name FROM result AS r INNER JOIN assigned_center AS a ON a.roll_no = r.roll_no INNER JOIN projects_posts AS pp ON pp.id = a.post_id INNER JOIN projects AS p ON p.id = pp.project_id INNER JOIN project_weightage AS pw ON pw.project_id = p.id INNER JOIN candidate_applied_post AS ca ON ca.id = a.cand_applied_id INNER JOIN candidates AS c ON c.id = ca.candidate_id LEFT JOIN district AS d ON d.id = c.district_id WHERE (ca.post_id = '$postId' OR '$postId' = 'all') AND c.gender = 'Male' AND (d.id = '$disId' OR '$disId' = 'all') AND p.id = '$proj_id' ORDER BY r.written_result DESC, pp.post_name, ca.id ASC";
        }
        elseif($gender == 'Female')
        {
        ...
        {
          $fetchData = "SELECT pw.`written_marks`, pw.`written_weightage`, pw.`typing_marks`, pw.`typing_weightage`, pw.`shorthand_marks`, pw.`shorthand_weightage`, c.`name`, c.`cnic`, c.`email`, c.`phone`, c.`password`, c.`image`, c.`status`, c.`signUpDate`, c.`f_name`, c.`gender`, c.`disability`, c.`disable_file`, c.`dob`, c.`city`, c.`postal_address`, c.`telephone`, c.`religion`, c.`gov_employee`, c.`simple_exper`, c.`retired_pak`, c.`army_exper`, c.`widow_gov_emp`, c.`widow_file`, c.`created_by`, c.`marital_status`, c.`father_occup`, c.`f_contact`, d.dis_name, r.roll_no, r.written_result, r.typing_result, r.shorthand_result, r.physical_result, pp.post_name FROM result AS r INNER JOIN assigned_center AS a ON a.roll_no = r.roll_no INNER JOIN projects_posts AS pp ON pp.id = a.post_id INNER JOIN projects AS p ON p.id = pp.project_id INNER JOIN project_weightage AS pw ON pw.project_id = p.id INNER JOIN candidate_applied_post AS ca ON ca.id = a.cand_applied_id INNER JOIN candidates AS c ON c.id = ca.candidate_id LEFT JOIN district AS d ON d.id = c.district_id WHERE (ca.post_id = '$postId' OR '$postId' = 'all') AND (d.id = '$disId' OR '$disId' = 'all') AND p.id = '$proj_id' ORDER BY r.written_result DESC, pp.post_name, ca.id ASC";
        }
      }
    ...
    <tr>
      ...
      <td><img src="<?php echo $imagePath; ?>" class="rounded" width="70px" height="70px"></td>
