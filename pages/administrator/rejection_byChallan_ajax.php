...
{
  $postId = $_POST['postId'];
  $candidate_type = $_POST['candidateType'];
  $where = "cap.post_id='$postId'";
  if($candidate_type == 'with-challan-tab') {
    $where .= " AND cap.challan_file IS NOT NULL";
  } else if($candidate_type == 'without-challan-tab') {
    $where .= " AND cap.challan_file IS NULL";
  }
