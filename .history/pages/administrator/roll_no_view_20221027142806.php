...
<section class="content">
		<div class="row">
			<div class="col-md-12 mb-2">
				<a class="printBlock btn btn-warning shadow float-left" href="search_roll_slip.php" >Back</a>
		...
		<div class="col-md-12">
			<div class="card">
				...
				<?php
					...
					$query = "SELECT p.project_name,p.project_id,c.cnic,c.name,c.f_name,c.image,ac.roll_no,pp.post_name,cs.reporting_date,cs.start_time,t.center_name,c.phone 
					FROM assigned_center AS ac INNER JOIN projects_posts AS pp ON pp.id = ac.post_id INNER JOIN projects AS p ON p.id = pp.project_id 
					INNER JOIN candidate_applied_post AS cap ON cap.id = ac.cand_applied_id INNER JOIN center_session AS cs ON cs.id = ac.session_id 
					INNER JOIN test_centers AS t ON t.id = cs.center_id INNER JOIN candidates AS c ON c.id = cap.candidate_id WHERE ac.roll_no = '$roll_no'AND c.id = '$cand_id'";
