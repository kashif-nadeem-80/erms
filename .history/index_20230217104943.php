            ...
            <div class="card-body">
              ...
              <form method="post" id="captcha_form">
                <div class="row">
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      <select type="text" name="office" class="form-control select1" required>
                        <option value="">select</option>
                        <?php
                        $fetchData = "SELECT name , dept_code  FROM department ORDER BY name ASC";
                        $runData = mysqli_query($connection,$fetchData);
                        while ($rowData = mysqli_fetch_array($runData)) {
                        $dept_id   = $rowData['dept_code'];
                        $dept_name = $rowData['name'];
                        ?>
...
if(isset($_POST['register']))
{
  ...
  $today          = date("Y-m-d");
  $fetch = "SELECT * FROM employees WHERE cnic = '$cand_cnic'";
  ...
  else
  {
    ...
    $insert = "INSERT INTO `employees`(`name`, `cnic`,`email`, `cellno`, `password`, `status`, `signUpDate`,`office`) 
    VALUES ('$cand_name','$cand_cnic','$cand_email','$cand_contact','$cand_password', '1','$today','$office')";
    ...
    if($run)
    {
      ...
      $htmlStr .= "<a href='http://www.pbm.gov.pk' target='_blank'>www.uts.com.pk</a><br />";
      $name = "Universal Testing Services";
      $email_sender = "info@uts.com.pk";
      $subject = "PBM | Sign Up | Confirmation of Email";
